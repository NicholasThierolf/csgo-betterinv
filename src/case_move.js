...
class items {
  ...
  getFullName(item) {
    if (item.item_wear_name) {
      return item.item_name + " (" + item.item_wear_name + ")";
    }
  ...
  inventoryConverter(inventoryResult, isCasket = false) {
    ...
    for (const [key, value] of Object.entries(inventoryResult)) {
      ...
      returnDict["market_hash_name"] = this.getFullName(returnDict);
      returnDict["in_container"] = isCasket;
      returnDict["container_id"] = value.casket_id;

      if (
        returnDict["item_name"] == "Alyx Pin" &&
        !returnDict["item_moveable"]
      ) {
        returnDict["market_hash_name"] = "Genuine Alyx Pin";
      }

      if (imageURL === "econ/music_kits/hlalyx_01") {
        //special case for HL Alyx music kit, which is unmovable
